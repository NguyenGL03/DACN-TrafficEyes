...
using gAMSPro.Localization.Dto;
using System;
...
namespace gAMSPro.Localization
{
    ...
    public class LanguageAppService : gAMSProAppServiceBase, ILanguageAppService
    {
        ...
        public async Task UpdateLanguageText(UpdateLanguageTextInput input)
        {
            ...
            switch (input.Mode)
            {
                case LanguageMethod.Add:
                    var languageText = _languageTextRepository.FirstOrDefault(x => x.Key == input.Key);
                    if (languageText == null)
                    {
                        await _languageTextRepository.InsertAsync(new ApplicationLanguageText()
                        {
                            Key = input.Key,
                            Value = input.Value,
