...
namespace gAMSPro.Authorization.Users
{
    ...
    public class UserAppService : gAMSProAppServiceBase, IUserAppService
    {
        ...
        {
            var query = await GetUsersFilteredQuery(input);
        ...
        //[AbpAuthorize(AppPermissions.Pages_Administration_Users_Create, AppPermissions.Pages_Administration_Users_Edit)]
        private async Task<IQueryable<User>> GetUsersFilteredQuery(IGetUsersInput input)
        {
            var userQuery = UserManager.Users.IgnoreQueryFilters();

            var currentUser = GetCurrentUser();
             
            List<string> branchs = null;

            if (input.SubbrId.IsNullOrWhiteSpace())
            {
                //input.SubbrId = currentUser.SubbrId;
            }

            if (input.IndependentUnit)
            {
            ...
            {
                branchs = (await _storeProcedureProvider
                .GetDataFromStoredProcedure<CM_BRANCH_GETCHILDIDModel>(CoreStoreProcedureConsts.CM_BRANCH_GETCHILDID, new { BRANCH_ID = input.SubbrId }))
            ...
            var query = UserManager.Users
                ...
                .WhereIf(!input.UserName.IsNullOrWhiteSpace(), u => u.UserName.Trim().ToLower().Contains(input.UserName.Trim().ToLower()))
                .WhereIf(!input.Filter.IsNullOrWhiteSpace(), u => u.UserName.Trim().ToLower().Contains(input.Filter.Trim().ToLower()))
                .WhereIf(!input.AUTH_STATUS.IsNullOrWhiteSpace(), u => u.AuthStatus.Trim().ToLower().Contains(input.AUTH_STATUS.Trim().ToLower()))
                .WhereIf(input.OnlyLockedUsers, u => u.LockoutEndDateUtc.HasValue && u.LockoutEndDateUtc.Value > DateTime.UtcNow);
                //.WhereIf(branchs != null, u => branchs.Contains(u.SubbrId))
                //.WhereIf(!input.DEP_ID.IsNullOrWhiteSpace(), u => u.DEP_ID.Trim().ToLower().Contains(input.DEP_ID.Trim().ToLower()));
