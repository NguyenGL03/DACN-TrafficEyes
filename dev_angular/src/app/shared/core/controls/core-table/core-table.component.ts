import { AfterContentChecked, AfterViewInit, ChangeDetectorRef, Component, ElementRef, EventEmitter, Input, OnInit, Output, ViewChild, ViewEncapsulation } from "@angular/core";
...
export class CoreTableComponent<T> implements OnInit, AfterViewInit, AfterContentChecked {
    ...
    ngAfterContentChecked(): void {
        ...
        $(this.tableRef?.nativeElement).find('tbody>tr').each(function (indexRow, valueRow) {
            $(valueRow).find('td').each(function (indexCol, valueCol) {
                if ($(valueCol).find(':input').length == 0) {
                    ...
                    if (colWidth / content.length < 10 || colWidth > 160) {
        ...
        //             var textContent = scope.encodeHTML($(value)
        //                 .clone()
        //                 .children()
        //                 .remove()
        //                 .end()
        ...
        //         });
        //     });
        //     $(this.tableRef?.nativeElement).each(function () {
