import { Component, Input, OnInit, Injector, ViewEncapsulation, Output, Inject, Optional, ViewChild, ElementRef, inject, OnDestroy, HostListener, OnChanges, SimpleChanges, EventEmitter } from "@angular/core";
...
import { FileUploaderMultiModalComponentOld } from "./file-uploader-multi-modal.component";
...
@Component({
    ...
    providers: [createCustomInputControlValueAccessor(FileUploaderOldComponent)]
})

export class FileUploaderOldComponent extends ControlComponent implements OnInit, OnDestroy {
    ...
    constructor(injector: Injector, @Optional() @Inject(API_BASE_URL) baseUrl?: string) {
        ...
        if ($('file-multi-modal').length == 0) {
            FileUploaderOldComponent.fileUploaderMultiModal = undefined;
        }

        if (FileUploaderOldComponent.fileUploaderMultiModal) {
    ...
    @ViewChild('fileMulti') fileMultiModal: FileUploaderMultiModalComponentOld;
    @ViewChild('control1') control1: ElementRef;
    static fileUploaderMultiModal: FileUploaderMultiModalComponentOld;
    ...
    afterViewInit() {
        ...
        if (!FileUploaderOldComponent.fileUploaderMultiModal) {
            FileUploaderOldComponent.fileUploaderMultiModal = this.fileMultiModal;
        }
        else {
            this.fileMultiModal = FileUploaderOldComponent.fileUploaderMultiModal;
        }
    ...
    @Input() @Output() public set ngModel(value) {
        ...
        if(this.control1?.nativeElement){
    ...
    ngOnDestroy(): void {
        // if(FileUploaderOldComponent.fileUploaderMultiModal){
        //     FileUploaderOldComponent.fileUploaderMultiModal.ngOnDestroy();
        // }
        //FileUploaderOldComponent.fileUploaderMultiModal = undefined;
        console.log('file picker destroy');

    }
    async fileUploadWord(el) {
        this.httpClient = this.injector.get(HttpClient);
        let scope = this;
        //let el = scope.fileControl.nativeElement;
        var MaxSize = Number.parseInt(this.s("gAMSProCore.FileSizeAttach"));
