...
export class Select2CustomComponent extends ControlComponent implements OnInit {
    ...
    afterViewInit() {
        ...
        // reload lai select 2 neu giao dien co
        let val = this.getValue(this.ngModel);
        if (val && $(this.inputRef.nativeElement).find('option[value="' + val.cdval + '"]').length) {
            this.inputRef.nativeElement.value = val.cdval;
            $(this.inputRef.nativeElement).trigger("change");
        }
    ...
    @Input() public set ngModel(value) {
        ...
        if (this.isNull(value) || this.isNull(val)) {
            // khi gia tri duoc chon khong nam trong list
