import { AfterContentChecked, AfterViewInit, ChangeDetectorRef, Component, ElementRef, EventEmitter, Input, NgZone, OnInit, Output, ViewChild, ViewEncapsulation } from "@angular/core";
...
export class EditableTableComponent<T extends Object> implements AfterViewInit, AfterContentChecked, OnInit {
            ...
            if ($(value).find(':input').length == 0) {
                ...
                if (colWidth / textContent.length < 10) {
                    ...
                    if (textContent.split(",").length > 1) {
                        var parsed = moment(textContent, "DD/MM/YYYY, hh:mm:ss a");
                        textContent = parsed.isValid() ? parsed.format("DD/MM/YYYY") : textContent;
                    } else {
                        var parsed = moment(textContent, "YYYY/MM/DD");
                        textContent = parsed.isValid() ? parsed.format("DD/MM/YYYY") : textContent;
                    }
                ...
                var textContent = '';
                textContent = scope.encodeHTML(($(value).find(':input:first')[0] as any).value);
                var colWidth = $(value).width();
                if (colWidth / textContent.length < 10) {
                    $(value).tooltip({
                        items: "td, input",
    ...
    removeAllCheckedItemWithCondition(field:string, condition:string) {
        let newData = this.allData.filter(x => !x['isChecked'] || x[field] == condition);
        this.allData = newData;
        this.resetNoAndPage();
        this.resetTableToClonePageRow();
        let pageSize = Math.min(this.tableState.defaultRecordsCountPerPage, this.tableState.allData.length);
    ...
    changePage(page, force: boolean = true, updateView = false, updateInput = true) {
        ...
        this.onChangePage.emit(undefined);
        this.checkShowErrorOnPage();
    ...
    getSumByCondition(fieldName, fieldName_condition, condition) {
        let val = 0;
        this.allData.filter(c => c[fieldName_condition] == condition).forEach(x => {
            val += (x[fieldName] || 0);
        })
        return val;
    }

    getSumByWithoutCondition(fieldName, fieldName_condition, condition) {
        let val = 0;
        this.allData.filter(c => c[fieldName_condition] != condition).forEach(x => {
            val += (x[fieldName] || 0);
        })
        ...
        if (tbody.getElementsByTagName('tr').length) {
            ...
            trs.each(function () {
                ...
                for (var inh = 0; inh < columns.length; inh++) {
                    ...
                    var mobileSpan = tds[inh].getElementsByClassName('mo-header') as any;
