import { Component, Input, OnInit, Injector, ViewEncapsulation, Output, ViewChild } from "@angular/core";
import {
    //LocationServiceProxy, CM_DISTRICT, CM_WARD, CM_NATION, CM_PROVINCE, CM_LOCATION_ENTITY
 } from "@shared/service-proxies/service-proxies";
import { Select2CustomComponent } from "../custom-select2/select2-custom.component";
import { ControlComponent, createCustomInputControlValueAccessor } from "../control.component-old";


declare var $: JQueryStatic;

@Component({
    selector: "location-control",
    templateUrl: "./location-control.component.html",
    encapsulation: ViewEncapsulation.None,
    providers: [createCustomInputControlValueAccessor(LocationControlComponent)]
})

export class LocationControlComponent extends ControlComponent implements OnInit {
    constructor(injector: Injector,
        // private locationService: LocationServiceProxy
    ) {
        super(injector);
    }

    // static location: CM_LOCATION_ENTITY;

    // nations: CM_NATION[];
    // provinces: CM_PROVINCE[];
    // districts: CM_DISTRICT[];
    // wards: CM_WARD[];

    district: string;
    ward: string;
    nation: string;
    province: string;

    @ViewChild('nationControl') nationControl: Select2CustomComponent;
    @ViewChild('provinceControl') provinceControl: Select2CustomComponent;
    @ViewChild('districtControl') districtControl: Select2CustomComponent;
    @ViewChild('wardControl') wardControl: Select2CustomComponent;

    @Input() disabled: boolean;

    @Input() swap = false;

    _ngModel: string;


    afterViewInit() {
        // COMMENT: this.stopAutoUpdateView();
        this.nation = 'VN';
        this.province = '79';
    }

    ngOnInit(): void {
        // if (!LocationControlComponent.location) {
        //     this.locationService.cM_LOCATION_AllData().subscribe(response => {
        //         LocationControlComponent.location = response;
        //         this.initLocation();
        //     })
        // }
        // else {
        //     this.initLocation();
        // }

    }

    private initLocation() {
        // this.nations = LocationControlComponent.location.natioNs;
        // LocationControlComponent.location.warDs.forEach(w => {
        //     let district = LocationControlComponent.location.districTs.firstOrDefault(x => x.diS_ID == w.diS_ID, {});
        //     let province = LocationControlComponent.location.provincEs.firstOrDefault(x => x.prO_ID == district.prO_ID, {});
        //     w['prO_ID'] = province.prO_ID;
        //     w['naT_ID'] = province.naT_ID;
        // });
        // LocationControlComponent.location.districTs.forEach(d => {
        //     let province = LocationControlComponent.location.provincEs.firstOrDefault(x => x.prO_ID == d.prO_ID, {});
        //     d['naT_ID'] = province.naT_ID;
        // });
        // if (this.ngModel) {
        //     this.setValueLocation(this.ngModel);
        // }
        // this.updateView();
    }

    public get ngModel(): any {
        return this._ngModel;
    }

    @Input() @Output() public set ngModel(value) {
        // if (LocationControlComponent.location && LocationControlComponent.location.natioNs) {
        //     this.setValueLocation(value);
        // }
        // else {
        //     this._ngModel = value;
        // }
    }

    getFirstOrDefault(list: any[], defaultValue = undefined) {
        if (list.length == 0) {
            return defaultValue;
        }
        return list[0];
    }

    setValueLocation(value) {
        if (!value) {
            return;
        }

        // let location = LocationControlComponent.location.warDs.firstOrDefault(w => w.waR_ID == value)
        //     || LocationControlComponent.location.districTs.firstOrDefault(w => w.diS_ID == value)
        //     || LocationControlComponent.location.provincEs.firstOrDefault(p => p.prO_ID == value)
        //     || LocationControlComponent.location.natioNs.firstOrDefault(n => n.naT_ID == value);

        // if (location['naT_ID']) {
        //     this.nation = location['naT_ID'];
        //     this.updateView();
        // }
        // if (location['prO_ID']) {
        //     this.province = location['prO_ID'];
        //     this.updateView();
        // }
        // if (location['diS_ID']) {
        //     this.district = location['diS_ID'];
        //     this.updateView();
        // }
        // if (location['waR_ID']) {
        //     this.ward = location['waR_ID'];
        //     this.updateView();
        // }
    }

    setNgModel(value) {
        this._ngModel = value;
        this.sendValueOut(value);
        this.updateView();
    }

    onChangeNation(nation: any) {
        // if (!nation) {
        //     return;
        // }
        // this.provinces = LocationControlComponent.location.provincEs.filter(x => x.naT_ID == nation.naT_ID);
        // this.districts = [];
        // this.wards = [];

        // this.setNgModel(nation.naT_ID);

        // this.provinceControl.ngModel = '';
        // this.districtControl.ngModel = '';
        // this.wardControl.ngModel = '';
    }

    onChangeProvince(province: any) {
        // if (!province) {
        //     return;
        // }
        // this.districts = LocationControlComponent.location.districTs.filter(x => x.prO_ID == province.prO_ID);
        // this.wards = [];
        // this.setNgModel(province.prO_ID);
        // this.districtControl.ngModel = '';
        // this.wardControl.ngModel = '';
    }

    onChangeDistrict(district: any) {
        // if (!district) {
        //     return;
        // }
        // this.wards = LocationControlComponent.location.warDs.filter(x => x.diS_ID == district.diS_ID);
        // this.setNgModel(district.diS_ID);
        // this.wardControl.ngModel = '';
    }

    onChangeWards(ward: any) {
        // if (!ward) {
        //     return;
        // }
        // this.setNgModel(ward.waR_ID);
    }
}
