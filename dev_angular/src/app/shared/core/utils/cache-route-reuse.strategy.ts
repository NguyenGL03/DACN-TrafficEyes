...
export class CacheRouteReuseStrategy implements RouteReuseStrategy {
    ...
    shouldReuseRoute(before: ActivatedRouteSnapshot, curr: ActivatedRouteSnapshot): boolean {
        ...
        if ((routeBefore + '-add' === routeCurrent || routeCurrent.startsWith(routeBefore + '-edit;') || routeCurrent.startsWith(routeBefore + '-view;'))
            && (routeBefore.startsWith('app/admin') || routeBefore.startsWith('app/main'))) {
            this.allowRetriveCache[routeBefore] = true;
        } else {
            this.allowRetriveCache[routeBefore] = false;
        }

        return before.routeConfig === curr.routeConfig || (routeBefore.startsWith('app/admin') && routeCurrent.startsWith('app/admin')) || (routeBefore.startsWith('app/main') && routeCurrent.startsWith('app/main'));
    }

    retrieve(route: ActivatedRouteSnapshot): DetachedRouteHandle | null {
        let url = this.getRoutePath(route);

        if (!this.allowRetriveCache[url]) {
            return undefined;
        }

        let result = this.storedRouteHandles.get(url) as DetachedRouteHandle;
        if (!result) {
            return undefined;
        }

        let component = this.storedRouteHandles.get(url)['componentRef'].instance;
        if (component['onRouteBack']) {
            component['onRouteBack']();
        }
