...
import { CoreTableComponent } from '@app/shared/core/controls/core-table/core-table.component';
import { EditableTableComponent } from '@app/shared/core/controls/editable-table/editable-table.component';
@Component({ template: '' })
export abstract class ListComponentBase<T> extends DefaultComponentBase implements AfterContentInit, IListComponent<T> {
    ...
    reloadPage(): void {
        if (this.pagingClient) {
            let page = this.dataTable.paginator.paginator.getPage();
            this.search();
    ...
    ngAfterContentInit(): void {
        ...
        // Paging Client Start
        if (this.pagingClient && this.dataTable?.onClickSorting) {
    ...
    pagingClient_ChangePage(page) {
        if (!this.clientData) {
            ...
            let pageSize = parseInt((this.dataTable.paginator.paginator.paginatorState.rows || 0));
            page = parseInt(page);
    ...
    onDblclick2(record: T) {
        if (this.appSession.user.roleName == 'GDDV' || this.appSession.user.roleName == 'GDK'
            || this.appSession.user.roleName == 'PTGD' || this.appSession.user.roleName == 'TPGD'
            || this.appSession.user.roleName == 'TKTGD'
            || this.appSession.user.roleName == 'TGD' || this.appSession.user.roleName == 'TKHDQT'
            || this.appSession.user.roleName == 'HDQT' || this.appSession.user.roleName == 'KSV'
            || this.appSession.user.roleName == 'TBP') {
            this.onViewDetail(record);
        }
        else if (this.appToolbar.buttonUpdateVisible) {
            this.onUpdate(record);
        }
        else if (this.appToolbar.buttonViewDetailEnable) {
            this.onViewDetail(record);
