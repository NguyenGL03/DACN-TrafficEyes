...
import { HostSettingsServiceProxy, ReportParameter, UiCustomizationSettingsDto } from '@shared/service-proxies/service-proxies';
import { FeatureCheckerService, LocalizationService, MessageService, NotifyService, PermissionCheckerService, SettingService } from 'abp-ng2-module';
import * as _ from 'lodash';
import * as moment from 'moment';
import { EditPageState } from './enum/edit-page-state';
@Injectable()
export abstract class ComponentBase extends AppComponentBase {
    ...
    constructor(injector: Injector) {
        ...
        this.outlet = injector.get(RouterOutlet);
    ...
    moneyTextToNumberFRound2(moneyText) {
        if (moneyText == '' || moneyText == null || moneyText === undefined) {
            return 0;
        }

        return this.round_or_0(parseFloat(moneyText.replace(/,/g, '')));
    }
    ...
    getFillterForCombobox(isApproveGroup?: boolean): any {
        return {
            maxResultCount: -1,
            recorD_STATUS: RecordStatusConsts.Active,
            autH_STATUS: isApproveGroup ? AuthStatusConsts.GroupApprove : AuthStatusConsts.Approve
        };
    }
    ...
    removeAllMessage() {
        var contentToRemove = document.querySelectorAll("#alert-message");
    ...
    showMessageMultiCondition(alertClass: string, message: string) {
        if (message.indexOf("\\n") >= 0) {
    ...
    showErrorMessageMultiCondition(message: string) {
        this.scrollTop();
        if (!$('#content').length) {
            // this.notify.error(message + '.');
        }
        this.showMessageMultiCondition('alert-danger', message);
        if (this.timeShowErrorMessage > 0) {
            setTimeout(() => {
                this.removeMessage();
            }, this.timeShowErrorMessage);
    ...
    showWarningMessageMultiCondition(message: string) {
        if (!$('#content').length) {
            // this.notify.warn(message + '.');
        }
        this.showMessageMultiCondition('alert-warning', message);
    }
    ...
    showSuccessMessageMultiCondition(message: string) {
        this.scrollTop();
        if (!$('#content').length) {
            // this.notify.success(message + '.');
        }
        this.showMessageMultiCondition('alert-success', message);
    }
    ...
    xlsRowsToArrCheckVal<T>(editable: any, rows: IterableIterator<Array<PrimeTableComponent<any>>>, xlsStructure: Array<string>, getObject = function (obj) { return obj; }, fieldCheckUnique: string = undefined, clearWaiting = true, sliceNum: number = 0): T[] {
        let i = 0;
        let uniqueList = [];
    ...
    xlsRowsToArr(rows: IterableIterator<Array<Object>>, xlsStructure: Array<string>, getObject = function (obj) { return obj; }, clearWaiting = true, sliceNum: number = 0) {
        ...
        let i = 0
        if (!curRow.value) {   // file import rỗng
            if (clearWaiting) {
                abp.ui.clearBusy();
            }
            this.showErrorMessage(this.l('File import không có dữ liệu'));
            return undefined;
        }
        else if (curRow.value && xlsStructure.length != curRow.value.length) {
            // cau truc file import khong dung voi xlsStructure
    ...
    getNumberAndCharacterFromString(str) {
        var numb = str.match(/[a-zA-Z0-9]/g);
        numb = numb.join("");
        return numb;
    }
    ...
    getMonthBetweenTwoDate(startDate: moment.Moment, endDate: moment.Moment) {
        let monthStart = startDate.month();
        let monthEnd = endDate.month();
        let yearStart = startDate.year();
        let yearEnd = endDate.year();
        let numberOfMonth = monthEnd - monthStart + 12 * (yearEnd - yearStart);

        return numberOfMonth;
    }

    showErrorMessageApprove(message: string) {
        this.scrollTop();
        if (!$('#message-approve').length) {
            // this.notify.error(message + '.');
        }
        this.showMessageApprove('alert-danger', message);
    ...
    showMessageApprove(alertClass: string, message: string) {
        this.removeMessageApprove();
        $('#message-approve').prepend('<div id="alert-message-approve" class="alert ' + alertClass + '" role="alert"><div class="alert-text">' + message + '.</div></div>');
    }

    removeMessageApprove() {
        $('#alert-message-approve').remove();
    }

    getState(): string {
        // console.log(this.outlet.activatedRouteData.permission)
        // Return a unique value based on the route's data or URL
        return this.outlet.activatedRouteData.permission;
    }
}
